...
| Importera `~/.ssh/config` → host-DB | ✅ testad (parser + dedup + `Include`/`Match`, LinuxApp läser filen från disk). Importerar även `ForwardAgent`, `RemoteCommand` → `startup_command` och `ProxyJump` → `jump_host_id` på BÅDA plattformarna (2026-08-19). `Host.forwardAgent` saknades helt i Swift-modellen och raderades därför vid varje synk genom en Apple-enhet — fältet finns nu, med ett test som kräver att nyckeln faktiskt skrivs ut |
