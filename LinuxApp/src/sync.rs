...
pub fn merge(a: SyncState, b: SyncState) -> SyncState {
    ...
    for h in a.hosts.into_iter().chain(b.hosts) {
        match newest_host.get(&h.id) {
            Some(existing) if h.modified_at.0 < existing.modified_at.0 => {}
            _ => {
    ...
    let all_ids: std::collections::HashSet<Uuid> =
        newest_host.keys().copied().chain(tomb.keys().copied()).collect();
    for id in all_ids {
    ...
    SyncState { hosts: live_hosts, tombstones: final_tombstones }
}
...
mod tests {
    ...
    fn newer_edit_wins_over_older_edit() {
        let id = Uuid::new_v4();
        let a = SyncState { hosts: vec![host_at("gammal", id, 10.0)], tombstones: HashMap::new() };
        let b = SyncState { hosts: vec![host_at("ny", id, 20.0)], tombstones: HashMap::new() };
    ...
    fn tombstone_wins_when_newer_than_edit() {
        let id = Uuid::new_v4();
        let a = SyncState { hosts: vec![host_at("host", id, 10.0)], tombstones: HashMap::new() };
        let mut tombstones = HashMap::new();
        tombstones.insert(id, ReferenceDate(20.0));
        let b = SyncState { hosts: vec![], tombstones };
        let merged = merge(a, b);
    ...
    fn newer_edit_revives_over_older_tombstone() {
        ...
        let a = SyncState { hosts: vec![], tombstones };
        let b = SyncState { hosts: vec![host_at("återupplivad", id, 20.0)], tombstones: HashMap::new() };
        let merged = merge(a, b);
        assert_eq!(merged.hosts.len(), 1);
    ...
    fn merge_is_commutative() {
        let id = Uuid::new_v4();
        let a = SyncState { hosts: vec![host_at("a", id, 10.0)], tombstones: HashMap::new() };
        let b = SyncState { hosts: vec![host_at("b", id, 20.0)], tombstones: HashMap::new() };
